...
class Terrain:
    ...
    def __init__(self, background_rect, rect_height, s=35):
        ...
        self.maxHeight = round(rect_height*random.uniform(0.5, 1)/2)
        self.smoothFactor = s
    ...
    def generateTerrain(self):
        ...
        for i in range(self.smoothFactor):
            ...
            for n in range(4, len(heights) - 3):
                heights[n] = sum(heights[n - 4: n + 5])/9
        # Since we coulnd't average heights[0], heights[1] and heights[-1] we do it outside of the loop
        for i in range(25):
            heights[0] = sum(heights[0: 9])/9
            heights[1] = sum(heights[0: 9])/9
            heights[2] = sum(heights[0: 9])/9
            heights[3] = sum(heights[0: 9])/9
            heights[len(heights) - 1] = sum(heights[len(heights) - 7: len(heights)])/9
            heights[len(heights) - 2] = sum(heights[len(heights) - 8: len(heights)-1])/9
            heights[len(heights) - 3] = sum(heights[len(heights) - 9: len(heights)-2])/9
        ...
        for width in range(len(self.ter)):
            for height in range(len(self.ter[width])):
                if self.ter[width][height] == 1:
                    ...
                    picture = TerrainSprite.TerrainSprite(width, height)
                    self.terrain_group.add(picture)
        
        return self.terrain_group